    ...
    return [
        ...
        },
        {
            ...
            content: `
                ...
                <p>Section 508 is available as an optional package at <strong>packages/accessibility-standards/section-508.package.json</strong>. In <strong>Settings &gt; Standards</strong>, check <strong>Section 508 Accessibility Standard</strong> to enable it and uncheck the same option to remove it. It remains separate from ART's built-in WCAG catalog.</p>
                <p>The Section 508 package provides concise original summaries and official source links for Revised 508 scope, chapters, functional performance criteria, testing, and Trusted Tester resources. It is an implementation aid, not legal advice, a conformance determination, or a certification. Qualified human review and any required Trusted Tester training or certification remain separate.</p>
                <p>The built-in <strong>Section 508 Accessibility Review</strong> template is available from <strong>File &gt; New &gt; Report &gt; New Report From Template</strong>, Dashboard template selection, and Report Builder template selection. It provides a starting structure for scope, requirements, testing, findings, results, and remediation evidence.</p>
                <p>In the Report Editor, each Section 508 audit field displays a visible field name beside its control and keeps a programmatic association to that visible label.</p>
                <p>After enabling Section 508, choose <strong>Section 508 Accessibility Standard</strong> in the Accessibility Lookup Tool's <strong>Standard</strong> filter to display its criteria. The lookup catalog refreshes when optional standards are enabled or removed.</p>
                <p>Section 508 Lookup results are individual expandable requirements rather than entries under ART's generic categories. They include FPC records, technical requirements, WCAG criteria, support requirements, Software requirements, and Hardware requirements, with requirement, scope/applicability, testing, result/documentation guidance, disability information when available, related WCAG references, and official source links.</p>
                <p>For Web Section 508 reports, Software and Support Documentation rows are excluded because they are not Web tests. The Web Editor also excludes the WCAG 1.1.1 through 4.1.3 range requested for this report workflow.</p>
                <p>The nine Functional Performance Criteria are individual requirements, not parent categories: <strong>302.1</strong> Without Vision, <strong>302.2</strong> With Limited Vision, <strong>302.3</strong> Without Perception of Color, <strong>302.4</strong> Without Hearing, <strong>302.5</strong> With Limited Hearing, <strong>302.6</strong> Without Speech, <strong>302.7</strong> With Limited Manipulation, <strong>302.8</strong> With Limited Reach and Strength, and <strong>302.9</strong> With Limited Language, Cognitive, and Learning Abilities.</p>
                <p>Section 508 Report Builder configuration requires <strong>Product Type</strong> Web, Electronic Document, Software, or Hardware, and <strong>Conformance Level</strong> Level A, Level AA, or Level AAA. Web and Software use separate supplied template resources. The shared <strong>Audit Start</strong> and <strong>Audit End</strong> fields provide the testing date range.</p>
                <p>When a Section 508 report is active in Report Editor, ART loads the supplied Web or Software template selected by Product Type and filters tests by Conformance Level. Every row includes a <strong>Test Result</strong> dropdown with <strong>Supports</strong>, <strong>Supports with Exceptions</strong>, <strong>Does Not Support</strong>, <strong>Not Applicable</strong>, and <strong>Not Evaluated</strong>, an <strong>Optional Comments</strong> textarea, and an <strong>Add Issue</strong> button.</p>
                <p>Section 508 <strong>Report Notes</strong> are entered in Report Editor. Audit Start and Audit End are the report-header dates, and the Editor does not duplicate a Report Date field. Criterion-specific Add Issue labels identify the test, while authoritative fixed results are locked, including Web test <strong>20.A 4.1.1-parsing</strong>, which is pre-filled as <strong>Not Evaluated</strong>.</p>
                <p>Repeated issue rows are numbered per criterion/test: the first row has the base label, followed by <strong>(2)</strong>, <strong>(3)</strong>, and later occurrence numbers.</p>
                <p>The supplied Section 508 Web and Software templates are preserved in the optional package, including their test IDs, names, conditions, product/system/tester structures, grouping, risk, and disability-impact metadata.</p>
                <p>The Section 508 report format follows the open ACR structure used by the <a href="https://acreditor.section508.gov/" target="_blank" rel="noopener noreferrer">GSA OpenACR Editor</a>. Fillable sections cover product and vendor identity, report status and contact details, evaluation scope, methods and evidence, environments, technologies, revision history, remarks, and additional buyer information. Informative guidance and official sources are included, and the Report Viewer publishes the ACR information and criteria results. ART does not claim to generate an official OpenACR YAML submission.</p>
