...
async function initTool() {
    ...
    if (LOADER_SRC) {
        dataUrlCandidates.push(new URL('wcag_data.js', LOADER_SRC).toString());
    }
...
const style = document.createElement('style');
    style.innerHTML = `
        details ul { list-style-type: none; padding-left: 0; margin: 0; } 
    ...
    <dl style="list-style-type: none; padding-left: 0;">
      <dt style="font-weight: bold; margin-bottom: 0.5em;">Instructions:</dt>
      <dd style="margin-left: 0; margin-bottom: 1em;">
        Use the search box to find specific criteria by name or keyword. Filters for version, level, and category will narrow down your results quickly.
      </dd>
      <dd style="margin-left: 0;">
        Each result can be expanded to view details, failures, and fixes. Use the "Copy" buttons to quickly grab text for your reports.
      </dd>
<dt style="font-weight: bold; margin-bottom: 0.5em;">
  <strong>Keyboard Shortcut:</strong></dt>
                    <dd style="margin-left: 0;">
                                                <strong>Reset Tool:</strong> Alt+Shift+D</dd>
                    </dl>
                    <p>
    <a href="https://github.com/ripplelearning/wcag-database" target="_blank" rel="noopener noreferrer">
    ...
    <details style="margin-bottom:10px; border:1px solid #eee;">
        <summary style="font-weight:bold; cursor:pointer; padding:10px;">${i.name} (Level ${i.level})</summary>
        <fieldset style="border:none; padding:10px; margin:0;">
            <dl style="list-style-type: none; padding-left: 0;">
                <dt style="font-weight: bold; margin-bottom: 0.5em;">Description:</dt>
                <dd style="margin-left: 0; margin-bottom: 1em;">${formatParagraphs(i.desc)}</dd>
                
                <dt style="font-weight: bold; margin-bottom: 0.5em;">Failures:</dt>
                <dd style="margin-left: 0; margin-bottom: 1em;">${formatAsList(i.failures)}</dd>
                
                <dt style="font-weight: bold; margin-bottom: 0.5em;">Fixes:</dt>
                <dd style="margin-left: 0; margin-bottom: 1em;">${formatAsList(i.fixes)}</dd>
                
                <dt style="font-weight: bold; margin-bottom: 0.5em;">Disabilities:</dt>
                <dd style="margin-left: 0; margin-bottom: 1em;">${formatAsCommaList(i.disabilitie)}</dd>
                
                <dt style="font-weight: bold; margin-bottom: 0.5em;">Link:</dt>
                <dd style="margin-left: 0; margin-bottom: 1em;">
                    <a href="${i.Link || '#'}" target="_blank">Open W3C documentation</a>
                </dd>
            </dl>
            <div style="margin-top:10px;">
                <ul style="list-style-type:none; padding:0;">
                    <li>  <button class="copy-btn" data-text="${i.name}\n\nDescription:\n${(i.desc || '').replace(/\|/g, ' ')}\n\nFailures:\n${(i.failures || '').replace(/\|/g, '\n')}\n\nFixes:\n${(i.fixes || '').replace(/\|/g, '\n')}\n\nDisabilities: ${formatAsCommaList(i.disabilitie)}\n\nLink: ${i.Link || 'N/A'}">Copy Full Entry</button></li>
                    <li><button class="copy-btn" data-text="${cleanForCopy(i.name)}">Copy Name</button></li>
                    <li><button class="copy-btn" data-text="${cleanForCopy(i.desc)}">Copy Desc</button></li>
                    <li><button class="copy-btn" data-text="${cleanForCopy(i.failures)}">Copy Failures</button></li>
                    <li><button class="copy-btn" data-text="${cleanForCopy(i.fixes)}">Copy Fixes</button></li>
                    <li><button class="copy-btn" data-text="${i.Link || ''}">Copy Link</button></li>
                </ul>
            </div>
        </fieldset>
    </details>
        ...
        const applyFilters = () => {
            ...
            render(data.filter(i => 
                ...
                (!c || ((Array.isArray(i.tags) ? i.tags : String(i.tags || '').split('|')).some(t => regex.test(t))) || (regex && (regex.test(i.name) || (i.desc && regex.test(i.desc)))))
            ));
        };
