...
Assert-True (@($standard.criteria | Where-Object { $_.number -match '^302\.[1-9]$' }).Count -eq 9) 'Section 508 package must contain exactly nine FPC records.'
Assert-True ((@($standard.criteria | Where-Object { $_.number -match '^302\.[1-9]$' } | ForEach-Object { $_.number }) -join ',') -eq '302.1,302.2,302.3,302.4,302.5,302.6,302.7,302.8,302.9') 'FPC identifiers are incomplete or out of order.'
Assert-True (@($standard.criteria | Where-Object { $_.number -match '^302\.[1-9]$' } | Where-Object { [string]::IsNullOrWhiteSpace($_.testingRequirements) -or [string]::IsNullOrWhiteSpace($_.testingGuidance) -or [string]::IsNullOrWhiteSpace($_.expectedResult) }).Count -eq 0) 'FPC testing metadata is incomplete.'
