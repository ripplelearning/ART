...
Section 508 Lookup results are individual expandable requirements, not members of ART's generic category headings. They include FPC records, technical requirements, WCAG success criteria, support requirements, Software requirements, and Hardware requirements. Each record shows its requirement, scope/applicability, testing requirements, how to test, expected/result guidance, how to document the result, disability information when available, related WCAG references when available, and an official documentation link.

In a Web Section 508 report, Software and Support Documentation rows are excluded because they do not apply to Web testing. The Web Editor also excludes the requested WCAG range from 1.1.1 through 4.1.3; remaining applicable WCAG and Section 508 test rows are preserved.

The nine Functional Performance Criteria are individual requirements: 302.1 Without Vision, 302.2 With Limited Vision, 302.3 Without Perception of Color, 302.4 Without Hearing, 302.5 With Limited Hearing, 302.6 Without Speech, 302.7 With Limited Manipulation, 302.8 With Limited Reach and Strength, and 302.9 With Limited Language, Cognitive, and Learning Abilities. They are not parent categories for technical requirements or tests.

Section 508 Report Builder configuration includes required **Product Type** values **Web**, **Electronic Document**, **Software**, and **Hardware**, plus required **Conformance Level** values **Level A**, **Level AA**, and **Level AAA**. The supplied Web and Software templates remain separate; the selected Product Type determines which template tests are loaded, and the selected Conformance Level determines which WCAG-level tests are shown. The shared **Audit Start** and **Audit End** fields provide the Section 508 testing date range in the report header.

When a Section 508 report is active in the Report Editor, ART loads the supplied Web or Software template selected by Product Type and filters its tests by Conformance Level. Each row includes a **Test Result** dropdown using **Supports**, **Supports with Exceptions**, **Does Not Support**, **Not Applicable**, and **Not Evaluated**, an **Optional Comments** textarea, and an **Add Issue** button.

Report-level **Notes** are entered in the Section 508 Report Editor, not Report Builder. The Editor does not duplicate the report date; Audit Start and Audit End are the report-header dates. Add Issue buttons are labeled with the exact criterion/test, such as **Add a row for 1.A Alt-version-conformant**. Tests with authoritative fixed results are locked; for example, Web test **20.A 4.1.1-parsing** is pre-filled as **Not Evaluated** and cannot be changed.

Adding another issue for a criterion/test keeps the base test label and numbers repeated rows: the first row is unsuffixed, the next is labeled `(2)`, then `(3)`, and so on.

The supplied templates are preserved as optional package resources at `packages/accessibility-standards/section-508-templates/`. Their source test identifiers, names, conditions, groups, risk fields, disability-impact fields, product fields, system fields, and tester fields remain available to the Section 508 workflow. ART does not claim that the supplied templates or generated reports establish legal compliance.
...
## Tasks and To-Do
Open Tasks and To-Do from Tools > Tasks and To-Do or Command Palette. Create local tasks and manage them with a native Complete checkbox and a status select directly on each task row, or select Edit on a task to open the Edit Task dialog and change the task name, priority, due date, and a single optional Comments text area.
...
The Dashboard To-Do List widget presents up to five active personal tasks, each with a Complete checkbox, task name, priority, and due date/time, without showing completed tasks. Local tasks do not create findings or declare an audit complete.

Account-specific assignment, shared-member selection, authorization, authenticated @mentions, and server notifications are deferred to Epics 49, 50, and 51. These capabilities must not be implemented with display names or email addresses alone.

## Organizations, Roles, and Device Identity
In Application Settings under Account and Identity, ART shows this installation's stable local device identity and a Personal ART Identity Code. The code is intended to recognize this installation when linking additional devices to the same identity; linking a different installation to a code requires ART's server-based identity service, which is not yet available, so Link Device only confirms whether an entered code matches this device's own code.
...
## Storage Providers
Application Settings includes a **Storage Providers** section listing every storage location ART's provider-independent storage architecture can work with. **Local Computer** and **Network or Shared Folder** are available today; both use the same operating-system file dialogs ART already relies on to open and save `.art` files, so ART never needs to know whether a folder is a plain local folder, a network drive, or a synchronized cloud-drive folder. **Dropbox** and **ART Server** are listed as planned providers; selecting Connect explains that the integration is not yet available and that ART remains fully usable with local files in the meantime.

Cloud providers such as **Google Drive** and **Microsoft OneDrive** are entirely optional. Their Dashboard buttons, File menu entries, Command Palette entries, and Keyboard Shortcut Manager rows stay hidden until you connect that specific provider in Application Settings; disconnecting hides them again. Nothing about the rest of ART changes based on which providers you have connected.
...
Google Drive connections are per-browser-session only in this release; reloading the page keeps the connection as long as the browser tab stays open, and closing the tab requires reconnecting.
...
### Microsoft OneDrive
Microsoft OneDrive works the same way as Google Drive. Connecting requires a Microsoft Azure AD Application (Client) ID, configured once by an ART administrator under **Microsoft OneDrive Connection** in Application Settings. Select **Connect OneDrive** to authorize through a Microsoft sign-in popup; allow popups for the ART site if the browser blocks it. ART requests only the `Files.ReadWrite.AppFolder` scope, limiting access to ART's own dedicated App Folder rather than your entire OneDrive. Select **Disconnect OneDrive** to end the session; this does not delete any OneDrive files.
