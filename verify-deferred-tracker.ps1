...
$missing = @($issues | Where-Object { $_.status -ne 'Deferred' -or [string]::IsNullOrWhiteSpace($_.documentationUrl) -or [string]::IsNullOrWhiteSpace($_.id) -or [string]::IsNullOrWhiteSpace($_.summary) })
...
Write-Host 'PASS: Every issue is Deferred with a stable ID and documentation link.'
